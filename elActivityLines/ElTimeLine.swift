...
open class elTimeline: UIScrollView {
    ...
    fileprivate func drawBubble(_ rect:CGRect, backgroundColor:UIColor, textColor:UIColor, titleLabel:UILabel) {
        ...
        if bubbleArrows {
            let startPoint = CGPoint(x: rect.origin.x, y: rect.origin.y + rect.height / 2 - 0)
            path.move(to: startPoint)
            path.addLine(to: startPoint)
            path.addLine(to: CGPoint(x: rect.origin.x - 10, y: rect.origin.y + rect.height / 2))
            path.addLine(to: CGPoint(x: rect.origin.x, y: rect.origin.y + rect.height / 2 + 0))
        }
